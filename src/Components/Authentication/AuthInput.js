...
const AuthInput = (props) => {
  ...
  const renderIcon1 = (props) => (
    <Pressable onPress={toggleSecureEntry1} style={{height:hp(5), display:'flex',justifyContent:'center',paddingHorizontal:hp(1)}}>
      <Entypo {...props} name={secureTextEntry1 ? 'eye-with-line' : 'eye'}  size={hp(2.5)} style={{color: passwordBorderColor}} />
    </Pressable>
  );

  const renderIcon2 = (props) => (
    <Pressable onPress={toggleSecureEntry2} style={{height:hp(5),display:'flex',justifyContent:'center',paddingHorizontal:hp(1)}}>
      <Entypo {...props} name={secureTextEntry2 ? 'eye-with-line' : 'eye'} size={hp(2.5)} style={{color: confirmBorderColor}} />
    </Pressable>
  );
