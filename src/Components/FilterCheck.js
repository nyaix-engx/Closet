...
const getColor=(index)=>{
    switch(props.category){
        case 'Size':
           return props.sizeFilters.includes(props.data[index].first) ? '#fb7ca0':'#c9c9c9'
        case 'Color':
            return props.colorFilters.includes(props.data[index].first) ? '#fb7ca0':'#c9c9c9'
        case 'Brand':
            return props.brandFilters.includes(props.data[index].first) ? '#fb7ca0':'#c9c9c9'
        default:
            return '#c9c9c9'
    }
}
...
return props.data.map((item,index)=>{
    return <Pressable onPress={()=>handlePress(index)} key={index} style={{height:hp(8)}}>
        <View style={{flex:1,display:'flex',flexDirection:'row',borderBottomWidth:hp(0.05),borderBottomColor:'#d1d1d1'}}>
            <View style={{flex:0.8,display:'flex',justifyContent:'center',alignItems:'center'}}>
                <Ionicons name="ios-checkmark" size={hp(3)} style={{color:getColor(index)}} />
            </View>
